...
from open_r1.trainer.vlm_seg_trainer import Qwen2VLGRPOVLLMTrainer
...
def initialize_tokenizer(model_path):
    ...
    if tokenizer is None:
        ...
        # Add SEG token if it doesn't exist
        tokenizer.add_special_tokens({'additional_special_tokens': ['<SEG>']})
        print(f"Added <SEG> token with ID: {tokenizer.convert_tokens_to_ids('<SEG>')}")
    return tokenizer
...
class GRPOScriptArguments(ScriptArguments):
    ...
    task_type: Optional[str] = field(
        ...
        metadata={"help": "Task type: 'segmentation', 'default', 'gui', ..."},
    )
    is_reward_customized_from_vlm_module: bool = field(
        default=False,
        metadata={"help": "Whether to use a customized reward from vlm module"},
    )
    seg_decoder_path: str = field(
        default="facebook/mask2former-swin-base",
        metadata={"help": "Path to segmentation decoder model"},
    )
    seg_token_id: Optional[int] = field(
        default=None,
        metadata={"help": "Token ID for segmentation tokens in the vocabulary"},
    )
...
def seg_iou_reward(content, sol, **kwargs):
    """Calculate IoU reward for segmentation masks."""
    # This is a placeholder for segmentation IoU calculation
    # In practice, you would need to parse mask predictions from content
    # and compare with ground truth masks in sol
    
    # For now, return a simple reward based on text similarity
    # This should be replaced with actual mask IoU calculation
    try:
        # Extract mask predictions from content (assuming JSON format)
        import json
        content_masks = json.loads(content) if content.strip().startswith('[') else []
        sol_masks = json.loads(sol) if sol.strip().startswith('[') else []
        
        if len(content_masks) == 0 and len(sol_masks) == 0:
            return 1.0
        elif len(content_masks) == 0 or len(sol_masks) == 0:
            return 0.0
        else:
            # Simple reward based on number of masks
            return min(len(content_masks) / len(sol_masks), len(sol_masks) / len(content_masks))
    except:
        return 0.0


def seg_accuracy_reward(completions, solution, **kwargs):
    """Reward function for segmentation accuracy."""
    contents = [completion[0]["content"] for completion in completions]
    rewards = []
    
    for content, sol in zip(contents, solution):
        reward = seg_iou_reward(content, sol)
        rewards.append(reward)
        ...
        if os.getenv("DEBUG_MODE") == "true":
            ...
            problem = kwargs.get("problem")[0]
            if reward <= 1.0:
                with open(log_path+"_seg.txt", "a", encoding='utf-8') as f:
                    f.write(f"------------- {current_time} Segmentation reward: {reward} -------------\n")
                    f.write(f"image_path: {image_path}\n")
                    f.write(f"problem: {problem}\n")
                    f.write(f"Content: {content}\n")
                    f.write(f"Solution: {sol}\n")
...
reward_funcs_registry = {
    ...
    "seg_iou": seg_iou_reward,
    "seg_accuracy": seg_accuracy_reward,
}
...
def get_vlm_module(model_name_or_path):
    if "qwen" in model_name_or_path.lower():
        if "seg" in model_name_or_path.lower():
            return QwenVLSegForConditionalGeneration
        else:
...
def main(script_args, training_args, model_args):
    ...
    # Handle segmentation model initialization
    if vlm_module_cls == QwenVLSegForConditionalGeneration:
        # For segmentation model, we need to initialize it with the decoder path
        from transformers import AutoConfig
        config = AutoConfig.from_pretrained(model_args.model_name_or_path)
        model = vlm_module_cls(config, seg_decoder_path=script_args.seg_decoder_path)
        
        # Initialize tokenizer and add SEG token
        ...
        if '<SEG>' in tokenizer.get_vocab():
            ...
            print(f"Using SEG token ID: {seg_token_id}")
            # Update the model's seg_token_id
            model.seg_token_id = seg_token_id
            # Update script args for consistency
            script_args.seg_token_id = seg_token_id
        
        # Set the model in the model_args for the trainer
        model_args.model = model
        question_prompt = "Please segment the objects in this image: {Question}"
    else:
    ...
    for data_file, image_folder, accu_reward_method in zip(data_files, image_folders, accu_reward_methods):
        with open(data_file, 'r') as f:
            for line in f:
                ...
                # Handle segmentation-specific data
                if script_args.task_type == "segmentation":
                    # Add segmentation masks if available
                    if 'masks' in item:
                        item['masks'] = item['masks']  # Keep as is, will be loaded as tensor later
                    else:
                        item['masks'] = None
                    
                    # Add class labels if available
                    if 'class_labels' in item:
                        item['class_labels'] = item['class_labels']  # Keep as is, will be loaded as tensor later
                    else:
                        item['class_labels'] = None
    ...
    def make_conversation_from_jsonl(example):
        if 'image_path' in example and example['image_path'] is not None:
            ...
            # Add SEG token for segmentation tasks
            if script_args.task_type == "segmentation":
                question_text = f"<SEG> {example['problem']}"
            else:
                ...
                'prompt': [{
                    ...
                    'content': [
                        *({'type': 'image', 'text': None} for _ in range(len(example['image_path']))),
                        {'type': 'text', 'text': question_prompt.format(Question=question_text)}
                    ]
                }]
            }
            
            # Add segmentation-specific data
            if script_args.task_type == "segmentation":
                if 'masks' in example and example['masks'] is not None:
                    result['masks'] = example['masks']
                if 'class_labels' in example and example['class_labels'] is not None:
                    result['class_labels'] = example['class_labels']
    ...
    # Select trainer class based on task type
    if script_args.task_type == "segmentation":
    ...
    if script_args.task_type == "segmentation":
        # For segmentation task, use the segmentation trainer
        trainer_kwargs = {
            'model': model_args.model if hasattr(model_args, 'model') else model_args.model_name_or_path,
        ...
        # Add SEG token ID if available
        if hasattr(script_args, 'seg_token_id') and script_args.seg_token_id is not None:
            trainer_kwargs['seg_token_id'] = script_args.seg_token_id
    else:
        # For other tasks, use the regular VLM trainer
        trainer_kwargs = {
